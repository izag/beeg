...
TIMEOUT = (3.05, 9.05)
SHORT_IMG_DELAY = 10000
LONG_IMG_DELAY = 60000
...
class MainWindow:
    ...
    def get_resolutions(self, for_record):
        ...
        # else:
        playlist_url = f"https://edge-hls.doppiocdn.live/hls/{self.model_name}/master/{self.model_name}_auto.m3u8"
    ...
    def fetch_image(self):
        ...
        image_url = f'https://img.doppiocdn.com/thumbs/{int(time.time())}/{self.model_name}_webp'

        try:
            # if self.img_counter % 30 == 0:
            #     self.img_url = HTTP_IMG_URL + self.model_name

            response = self.http_session.get(image_url, timeout=TIMEOUT)
...
class Chunks:
    ...
    def __init__(self, lines):
        ...
        self.header = None
        found = re.search(r"URI=\"(.*?)\"", lines[Chunks.IDX_HEADER], re.DOTALL)
        if (found is not None) and (found.group(0) is not None):
            ...
            try:
                ...
                for ts in chunks.ts:
                    ...
                    if RecordSession.SINGLE_THREAD:
                        ...
                        self.record_executor.submit(self.save_to_file, ts, filename)

                    self.file_num += 1

                    if (self.hist_logger is not None) and (self.file_num - 2) % 30 == 0:
...
if __name__ == "__main__":
    ...
    # session = RecordSession(None, None, 'lissinda_', 'https://b-hls-06.doppiocdn.live/hls/153128236/153128236_1080p60.m3u8', None, 0)
    # session.start()
    # session.join()

    # https://ru.sexvideochat.video/api/front/v2/models/username/Sexycikass/chat

# https://ru.sexvideochat.video/api/front/models?rcmGrp=A&improveTs=false&removeShows=false&srpm=0&limit=60&offset=0&primaryTag=girls&sortBy=stripRanking&excludeUsernames%5B0%5D=Anaruru222&userRole=guest&uniq=0fsk1wobxlm57e8c
# https://ru.sexvideochat.video/api/front/models/username/Anaruru222/related?limit=60&offset=0&primaryTag=girls&uniq=cxg2kunm1qzhs9t4