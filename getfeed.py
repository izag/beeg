...
class Room:
    def __init__(self, room_data):
        self.img_url = search(r'<img class="img-fluid" src="(.*?)"', room_data)
        slash_pos = self.img_url.rfind('/')
        self.model_name = self.img_url[slash_pos + 1: -4]
        self.bps = 0
        self.users = 0
        usr = search(r'([0-9]+)\s+польз', room_data)
        if len(usr) > 0:
            self.users = int(usr)
        self.log = ''
...
async def fetch_feed(start, finish):
    ...
    async with ClientSession(headers=HEADERS) as session:
        for page in range(start, finish + 1):
            url = f"https://sex-videochat.net/getfeeddata/chaturbate/ru/?page={page}"
